...
asBioData[asBioData.length] = ["Hilights","&bull;Published Author (Excellence in GUI Design) &nbsp; &nbsp; &bull;International conference speaker (User Experience)"];
asBioData[asBioData.length] = ["Skills"];
asBioData[asBioData.length] = ["Core Role","Senior JEE Analyst Programmer, Java Team Lead"];
asBioData[asBioData.length] = ["Technologies","Java J2EE, Agile (Scrum, RAD), Spring, Hibernate, Struts, Ajax, ReST, Oracle, XML"];
var sCerts = "<div class=\"bioData-valueColumn\">&bull;Sun Certified Java Programmer<br/>&bull;Sun Certified J2EE Web Component Developer";
sCerts += "</div><div class=\"bioData-valueColumn\">";
sCerts += "&bull;Adobe Flash Developer<br/>&bull;IBM Principal Application Developer (JavaScript)";
sCerts += "</div>";
sCerts += "";
sCerts += "";
asBioData[asBioData.length] = ["Certificates", sCerts];
...
sOut += timeline.getfunk6InFormat("<div id=\"timelineTable_${id}\" class=\"bioData-wrapper\" onmouseout=\"funk7(${id}, false)\" onmouseover=\"funk7(${id}, true);\"><div class=\"bioData-label\">${startReadable}-${endReadable}<br/>${durationReadable}</div><div class=\"bioData-value\">${title} <span class=\"bioData-value-description\">(${service})</span><div class=\"bioData-value-description\">${description}</div></div></div>");
...
funk3("fachEinsHeading", "Overview");
...
funk3("fachZweiHeading", "Tech Timeline");
funk3("fachDreiHeading", "Skills Radar", asTech);
funk3("fachVierHeading", "About&nbsp;");
}
function funk3(sAreaId, sText, asHints){
...
this.areaTrg.innerHTML = this.tofunk4() + this.toHeirarchy();
};
this.skill = function(sParent, sTitle, sDescription, iMagnitude, iX, iY, sColor){
iX = Math.round(iX + (this.widthGross / 2) - 44);
iY = Math.round(iY + (this.widthGross / 2) + 3);
this.skills[this.skills.length] = new Skill(sParent, sTitle, sDescription, iMagnitude, iX, iY, sColor);
};
...
funk0(this.skills, oRoot, "parentName", "name", 0);
};
this.sortByParent = function() {
return funk9(this.skills, "parentName,name", true);
};
this.tofunk4 = function() {
...
radar.skill("J2EE", "JSTL/JSF", "", 1, 135, -135, sColor);
