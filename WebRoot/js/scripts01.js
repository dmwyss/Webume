...
function writeBioData(){
	...
	sOut += timeline.getTimelineInFormat("<div id=\"timelineTable_${id}\" class=\"bioData-wrapper\" onmouseout=\"showHideBubble(${id}, false)\" onmouseover=\"showHideBubble(${id}, true);\"><div class=\"bioData-label\">${startReadable}-${endReadable}<br/>${durationReadable}</div><div class=\"bioData-value\">${title} <span class=\"bioData-value-description\">(${service})</span><div class=\"bioData-value-description\">${description}</div></div></div>");
...
function setUpFachHeadings(){
	...
	decorateFachBackground("fachZweiHeading", "Tech Timeline");
...
function continueAnimBlipCircle(){
	if(iAnimSize > 240){ // Size up to 120.
		stopAnimBlipCircle();
	}
	iAnimSize += 6;
 	resetAnimBlipCircle();
