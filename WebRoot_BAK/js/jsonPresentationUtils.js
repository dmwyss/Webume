...
function jsonToFormat(o, sFormat) {
	//toFormat("${parentName}, ${name}, ${rating}");
...
function jsonToHeirarchyFormat(oParent, sFormat, sChildrenWrapperStart, sChildrenWrapperEnd, sOut) {
	// Get everything with whose parent name matches sParentValue
	var sOutTemp = "";
	//sOutTemp += jsonRepeat(sLeadRepeater, oParent.depth) + jsonToFormat(oParent, sFormat);
	/*
	if(oParent.children.length > 0){
		sOutTemp += sFolderLeadHtml;
	} else {
		sOutTemp += sFileLeadHtml;
	}
	*/
...
/*
function jsonRepeat(sIn, iRepeat){
	var sOut = ""
	for(var iCount = 0; iCount < iRepeat; iCount++){
		sOut += sIn;
	}
	return sOut;
}
